...
// --- Begin integrated closure allocation for all architectures ---
...
ffi_closure_alloc (size_t size, void **code)
{
    ...
    // Выделяем память под трамплин + структуру ffi_closure
#if defined(ESP_PLATFORM) && defined(CONFIG_LIBFFI_USE_IRAM_POOL)
    mem = iram_pool_alloc(FFI_TRAMPOLINE_SIZE + size);
#else
    mem = malloc(FFI_TRAMPOLINE_SIZE + size);
#endif
    if (!mem)
        return NULL;

    // Трамплин в начале
    // Структура ffi_closure следует сразу за трамплином
    closure = (char*)mem + FFI_TRAMPOLINE_SIZE;

    // Возвращаем адрес начала структуры closure (где находится tramp)
    *code = closure;  // Для Xtensa closures, *code должен указывать на closure->tramp
...
ffi_closure_free (void *ptr)
{
    if (!ptr)
        return;
#if defined(ESP_PLATFORM) && defined(CONFIG_LIBFFI_USE_IRAM_POOL)
    // Освобождаем как IRAM, так и fallback-EXEC
    void *mem = (char*)ptr - FFI_TRAMPOLINE_SIZE;
    iram_pool_free(mem);
#else
    void *mem = (char*)ptr - FFI_TRAMPOLINE_SIZE;
    free(mem);
...
// --- End integrated closure allocation ---
...
ffi_closure_alloc (size_t size, void **code)
{
  ...
  // Выделяем память под структуру ffi_closure
  // Для RISC-V: closure содержит tramp[24] + cif + fun + user_data
#if defined(ESP_PLATFORM) && defined(CONFIG_LIBFFI_USE_IRAM_POOL)
  closure = iram_pool_alloc (size);
#else
  closure = malloc (size);
#endif
  ...
  // Для RISC-V: исполняемый код находится в начале структуры closure (tramp)
  *code = closure;  // code указывает на closure->tramp

  return closure;   // возвращаем указатель на структуру closure
}
...
void
ffi_go_closure_free (void *closure)
{
